...
# corrected data.
# Mathieu Fortin - July 2023, Nov 2024
#   @output QcPSP.Rds
###########################################
...
load(file = "./compilation/ReadTrees.RData")
...
output <- readRDSFile()
if ("plots" %in% names(output)) {
  compareTwoDataFrame(QcPlotIndex, output$plots)
}
...
write.csv(trees, file = file.path(getwd(), "compilation", "treesBeforeCorrection.csv"), row.names = F)
...
classPath <- c(file.path(getwd(), "compilation", "JavaProcessing/bin/"), file.path(getwd(), "compilation", "JavaProcessing/ext/*"))
connectToJava(extensionPath = classPath, memorySize = 3000)
#getClassLoaderPaths()

treeBeforeCorrectionFilename <- file.path(getwd(), "compilation", "treesBeforeCorrection.csv")
...
exportCorrectedFilename <- file.path(getwd(),"compilation", "treesCorrected.csv")
dsgm$save(exportCorrectedFilename)
shutdownClient()


#### Creating tree index table after Java processing ####
correctedTreeMeasurements <- read.csv(file = "./compilation/treesCorrected.csv", sep=";", dec=".")  ### 2 089 280 obs
...
output <- readRDSFile()
if ("treeIndex" %in% names(output)) {
  compareTwoDataFrame(treeIndex, output$treeIndex)
}
...
correctedTreeMeasurementIntermediatefilename <- file.path(getwd(), "compilation", "correctedTreeMeasurementsIntermediate.RData")

save(QcTreeMeasurements, file = correctedTreeMeasurementIntermediatefilename)
save(measurementIndex, file = "./compilation/measurementIndexIntermediate.RData")

############### Summarizing basal areas and stem density per measurements in the whole plot (400 m2 and 625 m2 subplots) ###########

rm(list=ls())
source("./compilation/utilityFunctions.R")
load(file = "./compilation/correctedTreeMeasurementsIntermediate.RData")
load(file = "./compilation/measurementIndexIntermediate.RData")
...
output <- readRDSFile()
if ("treeMeasurements" %in% names(output)) {
  compareTwoDataFrame(QcTreeMeasurements, output$treeMeasurements)
}
...
output <- readRDSFile()
if ("plotMeasurements" %in% names(output)) {
  compareTwoDataFrame(QcMeasurementIndex, output$plotMeasurements)
}
...
if ("saplings" %in% names(output)) {
  compareTwoDataFrame(QcSaplingMeasurements, output$saplings)
}
...
load(file = "./compilation/ReadTrees.RData")
output <- readRDSFile()
if ("photoInterpretedStands" %in% names(output)) {
  compareTwoDataFrame(inventory$photoInterpretedStands, output$photoInterpretedStands)
}
